...
pwt = pd.read_excel("pwt/pwt110.xlsx", sheet_name = "Data")

pwt_ids = ["countrycode", "country", "currency_unit", "year"]
pwt_vars = ["labsh", "avh", "cn", "ctfp", "rtfpna", "csh_c", "csh_i", "csh_g", "cgdpo", "delta"]
pwt = pwt[pwt_ids + pwt_vars].query(f"year=={year_analysis_pwt}")
...
oecd_fpi = oecd_fpi[oecd_fpi.Country.isin(['Chile', "Costa Rica", "Mexico"])].query(f"Year == {year_analysis_oecd_fpi}")[["Country","Year","Value"]]
...
oecd_pep = pd.read_csv("oecd/founded_pensions_indicators/public_expenditure_on_pensions/public_expenditure_pensions_2019.csv")
oecd_pep = oecd_pep[oecd_pep.Country.isin(['Chile', "Costa Rica", "Mexico"])][["Country", "2019"]].rename(columns = {"Country" : "countrycode", "2019" : "public_expenditure_pensions_perc_gdp"})
...
oecd_rs_total_tax = oecd_rs[(oecd_rs["Tax revenue"]=='1000 Taxes on income, profits and capital gains') | (oecd_rs["Tax revenue"]=='5000 Taxes on goods and services')].query(f"GOV == 'FED' and VAR =='TAXPER' and Year =={year_analysis_oecd_rs}")
...
oecd_tax_db = pd.read_csv("tax_data/tax_data.csv")
...
oecd_tax_db = oecd_tax_db.query(f"year=={year_analysis_oecd_tax_db}")
